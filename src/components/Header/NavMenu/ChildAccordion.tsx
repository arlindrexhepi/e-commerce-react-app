import { Link } from "react-router-dom";
import { ParentAccordionProps } from "./ParentAccordion";

interface ChildAccordionProps {
  showAccordion: ParentAccordionProps["showAccordion"];
  accordionImages: ParentAccordionProps["accordionImages"];
  onSetShowAccordion: ParentAccordionProps["onSetShowAccordion"];
}

const ChildAccordion: React.FC<ChildAccordionProps> = ({
  showAccordion,
  accordionImages,
  onSetShowAccordion
}) => {
  return (
    <div
      className={`${
        showAccordion ? "visible opacity-100" : "invisible opacity-0"
      } 
          childAccordion absolute bg-white px-8 py-4 top-[100%] w-full flex 
          items-center rounded-b-lg shadow-lg text-black`}
      onMouseEnter={() => onSetShowAccordion(true)}
      onMouseLeave={() => onSetShowAccordion(false)}
    >
      <div className="w-[80%] grid grid-cols-3">
        <div>
          <p className="text-lg font-semibold text-darkPrimary">
            <Link to="/">Televisions</Link>
          </p>
          <ul>
            <li>
              <Link to="/">Smart TVs</Link>
            </li>
            <li>
              <Link to="/">4K TVs</Link>
            </li>
            <li>
              <Link to="/">Full HD TVs</Link>
            </li>
            <li>
              <Link to="/">Large Screen TVs</Link>
            </li>
            <li>
              <Link to="/">32 inch TVs</Link>
            </li>
          </ul>
        </div>
        <div>
          <p className="text-lg font-semibold text-darkPrimary">
            <Link to="/">Shop by Brand</Link>
          </p>
          <ul>
            <li>
              <Link to="/">Samsung</Link>
            </li>
            <li>
              <Link to="/">LG</Link>
            </li>
            <li>
              <Link to="/">Sony</Link>
            </li>
            <li>
              <Link to="/">Philips</Link>
            </li>
            <li>
              <Link to="/">Panasonic</Link>
            </li>
          </ul>
        </div>
        <div>
          <p className="text-lg font-semibold text-darkPrimary">
            Other Home Entertainment
          </p>
          <ul>
            <li>
              <Link to="/">Home Theater Systems</Link>
            </li>
            <li>
              <Link to="/">Projectors &amp; Accesories</Link>
            </li>
            <li>
              <Link to="/">Set top Boxes</Link>
            </li>
            <li>
              <Link to="/">Streaming Media Players</Link>
            </li>
            <li>
              <Link to="/">DVD &amp; Blu-ray Players</Link>
            </li>
            <li>
              <Link to="/">Cables</Link>
            </li>
            <li>
              <Link to="/">Audio-video Accessories</Link>
            </li>
          </ul>
        </div>
        <div>
          <p className="text-lg font-semibold text-darkPrimary">
            Home Audio/Video
          </p>
          <ul>
            <li>
              <Link to="/">Speakers</Link>
            </li>
            <li>
              <Link to="/">Home Theaters</Link>
            </li>
            <li>
              <Link to="/">Projectors</Link>
            </li>
            <li>
              <Link to="/">Headphones</Link>
            </li>
            <li>
              <Link to="/">Noise Cancelling Headphones</Link>
            </li>
            <li>
              <Link to="/">Soundbars</Link>
            </li>
            <li>
              <Link to="/">Headphones with Mic</Link>
            </li>
          </ul>
        </div>
        <div>
          <p className="text-lg font-semibold text-darkPrimary">
            Shop by Brand
          </p>
          <ul>
            <li>
              <Link to="/">JBL</Link>
            </li>
            <li>
              <Link to="/">Sony</Link>
            </li>
            <li>
              <Link to="/">Sennheiser</Link>
            </li>
            <li>
              <Link to="/">Bose</Link>
            </li>
            <li>
              <Link to="/">AKG</Link>
            </li>
            <li>
              <Link to="/">SteelSeries</Link>
            </li>
            <li>
              <Link to="/">Razer</Link>
            </li>
          </ul>
        </div>
      </div>
      <div className="w-[20%]">
        {accordionImages.map((el: string) => {
          return <img key={el} className="max-w-[75%]" src={el} alt={el} />;
        })}
      </div>
    </div>
  );
};

export default ChildAccordion;
