export default {
  categories: [
    {
      name: "TV & Audio",
      childs: [
        {
          name: "Television",
          subChilds: [
            "Smart TVs",
            "4K TVs",
            "Full HD TVs",
            "Large Screens TVs",
            "32 inch TVs",
            "48 inch TVs & Above"
          ]
        },
        {
          name: "Shop TVs by Brand",
          subChilds: ["VU", "Sony", "LG", "Samsung", "Vivax", "Fuego"]
        },
        {
          name: "Other Home Entertainment",
          subChilds: [
            "Home Theater Systems",
            "Projectors & Accessories",
            "Set top Boxes",
            "Streaming Media Players",
            "DVD & Blu-Ray Players",
            "Cables",
            "Audio-video Accessories"
          ]
        },
        {
          name: "Home Audio/Video",
          subChilds: [
            "Speakers",
            "Home Theaters",
            "Projectors",
            "Headphones",
            "Noise Cancelling Headphones",
            "Soundbars",
            "Headphones with Mic"
          ]
        },
        {
          name: "Shop Audio by Brand",
          subChilds: [
            "JBL",
            "Senheiser",
            "Sony",
            "Philips",
            "Harmon Kardon",
            "Protonics",
            "Beats by Dre"
          ]
        }
      ],
      images: [
        "https://i.imgur.com/Ry0CTCY.png",
        "https://i.imgur.com/ZIVl09p.png"
      ]
    },
    {
      name: "Smartphones",
      childs: [
        {
          name: "Accessories",
          subChilds: [
            "Headsets",
            "Cables & Chargers",
            "Electronic Accessories",
            "Selfie Sticks",
            "Internal Batteries",
            "Motorola",
            "Nokia"
          ]
        },
        {
          name: "Tablets",
          subChilds: ["iPads", "Samsung", "Microsoft Surface", "Lenovo"]
        },
        {
          name: "Mobiles",
          subChilds: [
            "Samsung",
            "Lenovo",
            "Mi",
            "Motorola",
            "Oppo",
            "Panasonic",
            "HTC",
            "Blackberry",
            "LG"
          ]
        },
        {
          name: "#Trending",
          subChilds: [
            "Oppo",
            "Panasonic",
            "Samsung",
            "Lenovo",
            "Mi",
            "Huawei",
            "HTC",
            "LG"
          ]
        },
        {
          name: "Cases & Covers",
          subChilds: [
            "For iPhone X",
            "For Samsung S21",
            "Below AED 59",
            "For Xiaomi",
            "For iPhone 7"
          ]
        }
      ],
      images: [
        "https://i.imgur.com/fjSK7GJ.png",
        "https://i.imgur.com/g9JmvS3.png"
      ]
    },
    {
      name: "Laptops & Desktops",
      childs: [
        {
          name: "Laptops",
          subChilds: [
            "Thin and Light Laptops",
            "2-in-1 Laptops",
            "Gaming Laptops",
            "Budget Laptops",
            "Business Laptops"
          ]
        },
        {
          name: "Tablets",
          subChilds: ["Lenovo", "Apple", "Samsung", "Micromax", "iBall"]
        },
        {
          name: "Processors",
          subChilds: ["Intel", "AMD", "Qualcomm", "IBM", "Motorola"]
        },
        {
          name: "Best Processors",
          subChilds: [
            "Intel Core i9-7980XE",
            "AMD Ryzen Threadripper 1950X",
            "AMD Ryzen 5 1600X",
            "Intel Core i7-8700K",
            "AMD Ryzen 7 1800K",
            "Intel core i5-7600K",
            "AMD Ryzen 3 1300X"
          ]
        },
        {
          name: "Shop by Brand",
          subChilds: [
            "Apple",
            "HP",
            "Lenovo",
            "Dell",
            "Legion",
            "Alienware",
            "Razer"
          ]
        },
        {
          name: "PC Accessories",
          subChilds: [
            "Keyboards",
            "Mice",
            "Laptop Bags",
            "PC Speakers",
            "Laptop Batteries"
          ]
        }
      ],
      images: [
        "https://i.imgur.com/8ZXwLhy.png",
        "https://i.imgur.com/C7OTEnR.png"
      ]
    },
    {
      name: "Gadgets",
      childs: [
        {
          name: "Computers & Accessories",
          subChilds: [
            "Laptops, Desktops & Monitors",
            "Networking & Internet Devices",
            "Computer Accessories"
          ]
        },
        {
          name: "Cameras",
          subChilds: [
            "DSLR",
            "Lenses",
            "Security & Surveillance",
            "Binoculars & Telescopes",
            "Camcorders"
          ]
        },
        {
          name: "Accessories",
          subChilds: [
            "Mouses",
            "Keyboards",
            "Hardrives",
            "Headphones",
            "Speakers"
          ]
        },
        {
          name: "Peripherals",
          subChilds: [
            "Hard Drives",
            "Pen Drives & Memory Cards",
            "Printers & Ink",
            "Mouse"
          ]
        },
        {
          name: "Watches",
          subChilds: [
            "Apple Smartwatches",
            "Samsung Smartwatches",
            "Fossil Smartwatches",
            "Other Smartwatches",
            "Men's Smartwatches",
            "Women's Smartwatches",
            "Deals on Watches"
          ]
        },
        {
          name: "Gadgets",
          subChilds: [
            "Fire TV Stick",
            "Google Chromecast",
            "Set Top",
            "Accessories",
            "Deals of the Day"
          ]
        }
      ],
      images: [
        "https://i.imgur.com/VoF4Re9.png",
        "https://i.imgur.com/mZMkSwX.png"
      ]
    },
    {
      name: "GPS & Car",
      childs: [
        {
          name: "Car Audios",
          subChilds: [
            "Music Systems",
            "Video System",
            "Speakers",
            "Car Monitors",
            "Car Parts"
          ]
        },
        {
          name: "GPS & Accessories",
          subChilds: [
            "GPS Tracking Devices",
            "GPS Networking Devices",
            "GPS Tracking Sensors",
            "GPS Monitoring",
            "GPS Jammers"
          ]
        },
        {
          name: "Accessories & Parts",
          subChilds: [
            "Car Accessories",
            "Car Electronics",
            "Car Parts",
            "Car & Bike Care",
            "All Car Products"
          ]
        },
        {
          name: "Shop by Brand",
          subChilds: [
            "Sony",
            "Pioneer",
            "JBL",
            "Kenwood",
            "Clarion",
            "Alpine",
            "Bang & Olufsen",
            "Blaupunkt",
            "Bose"
          ]
        },
        {
          name: "Car Audios Cables",
          subChilds: [
            "Car Accessories",
            "Car Electronics",
            "All Audio Products",
            "Accessories",
            "Network Jammers",
            "Car Parts"
          ]
        },
        {
          name: "GPS",
          subChilds: [
            "GPS Tracking Sensors",
            "GPS Monitoring",
            "GPS Anti-Tracker",
            "GPS Accessories",
            "GPS Devices",
            "GPS Trackers"
          ]
        }
      ],
      images: [
        "https://i.imgur.com/LgcmHqX.png",
        "https://i.imgur.com/tUfZWY1.png"
      ]
    },
    {
      name: "Cameras & Accessories",
      childs: [
        {
          name: "Cameras",
          subChilds: [
            "DSLR Cameras",
            "Digital Cameras",
            "Security & Surveillance",
            "Camcorders",
            "Consoles"
          ]
        },
        {
          name: "Shop by Focal Length",
          subChilds: [
            "8mm - 24mm",
            "24mm - 35mm",
            "35mm - 85mm",
            "85mm - 135mm",
            "135mm+"
          ]
        },
        {
          name: "Accessories",
          subChilds: [
            "Headphones with Mic",
            "Mouses",
            "Hardrives",
            "Headphones",
            "Speakers"
          ]
        },
        {
          name: "Shop by Brand",
          subChilds: [
            "Sony",
            "Canon",
            "Nikon",
            "Pentax",
            "Apple",
            "Leica",
            "Samsung",
            "Panasonic",
            "LG",
            "Oppo",
            "Olympus"
          ]
        },
        {
          name: "#Trending",
          subChilds: ["Sony", "Nikon", "Canon", "Sanyo", "Samsung"]
        },
        {
          name: "Add-ons",
          subChilds: [
            "Data Cables",
            "Keypads",
            "Earphones",
            "Lenses",
            "Camera Accessories"
          ]
        }
      ],
      images: [
        "https://i.imgur.com/el1lVgG.png",
        "https://i.imgur.com/OArMZSG.png"
      ]
    },
    {
      name: "Movies & Games",
      childs: [
        {
          name: "Movies & TV Shows",
          subChilds: [
            "All Movies & TV Shows",
            "Blu-ray",
            "Latest Movies",
            "All English",
            "All German",
            "All Spanish"
          ]
        },
        {
          name: "Video Games",
          subChilds: [
            "Games & Accessories",
            "PC Games",
            "New Releases",
            "Consoles",
            "Accessories"
          ]
        },
        {
          name: "Music",
          subChilds: [
            "5.1 Speakers",
            "Home Theatres",
            "Soundbars",
            "Accessories",
            "Consoles"
          ]
        }
      ],
      images: [
        "https://i.imgur.com/fzqy2F3.jpg",
        "https://i.imgur.com/NWQFu35.png"
      ]
    }
  ]
};
